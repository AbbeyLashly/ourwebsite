        ...
        if ($this->is_review and $show_stars_on_review === true) {
            ...
            if (td_util::get_option('tds_p_show_date') != 'hide') {
                ...
                $buffy .= '<div class="td-post-date">';
                    ...
                    $buffy .= '<time  itemprop="dateCreated" class="entry-time updated td-module-date' . $visibility_class . '" datetime="' . date(DATE_W3C, $td_article_date_unix) . '" >' . get_the_time(get_option('time_format'), $this->post->ID) . '</time>';
